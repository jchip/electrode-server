    ...
    const doRequire = () => {
      ...
      return Promise.try(() => xrequire(name))
        ...
        .then(mod => {
          ...
          // 3. mod.default (ES6)
          // 4. mod
          const plugin = ["hapiPlugin", "default.hapiPlugin", "default"].find(x => _.get(mod, x));
  ...
  const registerPlugins = plugins => {
    ...
    return Promise.each(plugins, plg => {
      ...
      return registerPromise.catch(err => {
        const originalErr = context.preserveError;
        if (!err || !err.hasOwnProperty("message")) {
          originalErr.message = err;
        } else if (err instanceof Promise.TimeoutError) {
          originalErr.message = `timeout - did you return a resolved promise?`;
        } else {
          originalErr.message = err.message;
        }

        originalErr.code = "XPLUGIN_FAILED";
        originalErr.plugin = plg;
        originalErr.method = registerMethod(plg);
        originalErr._err = err;
